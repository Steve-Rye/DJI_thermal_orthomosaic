...
class ImageProcessor:
    ...
    def _process_single_folder(self, folder_path: str) -> None:
        ...
        pos_dir = self._create_directory(folder_path, self.POS_DIR_NAME)
        temp_dir = self._create_directory(folder_path, self.TEMP_DIR_NAME)

        try:
            # 整理文件并处理
            self._move_files_and_process(folder_path, input_dir, output_dir, pos_dir, temp_dir)
        finally:
            # 清理临时目录
    ...
    def _move_files_and_process(self, src_dir: str, input_dir: str, output_dir: str, 
        ...
        # 移动POS文件
        pos_file = os.path.join(src_dir, "pos.txt")
        if os.path.exists(pos_file):
            shutil.move(pos_file, os.path.join(pos_dir, "pos.txt"))
            self._process_pos_file(pos_dir)
    ...
    def _process_pos_file(self, pos_dir: str) -> None:
        """处理POS文件，提取关键信息并重新格式化"""
        pos_file = os.path.join(pos_dir, "pos.txt")
        if not os.path.exists(pos_file):
            return

        output_file = os.path.join(pos_dir, "posT.txt")
        
        with open(pos_file, 'r', encoding='utf-8') as infile, \
             open(output_file, 'w', encoding='utf-8') as outfile:
            for line in infile:
                if "D" in line:  # 只处理包含图像信息的行
                    processed_line = self._process_pos_line(line)
                    outfile.write(processed_line + "\n")

    def _process_pos_line(self, line: str) -> str:
        """处理单行POS数据"""
        parts = line.split(',')
        if len(parts) < 9:
            raise ValueError(f"无效的POS数据格式: {line}")

        name = parts[0][-29:-4]     # 提取文件名（不含扩展名）
        lat = parts[1]              # 纬度
        lon = parts[2]              # 经度
        alt = parts[3]              # 高度
        yaw = parts[5]              # 偏航角
        pitch = parts[6]            # 俯仰角
        roll = parts[7]             # 翻滚角
        timestamp = parts[8].strip() # 时间戳

        return f"{name}.JPG,{lat},{lon},{alt},{yaw},{pitch},{roll},{timestamp}"
